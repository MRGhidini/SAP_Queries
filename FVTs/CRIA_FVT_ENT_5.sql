...
--
--	Substituições(CTRL H)		: _ENT para _xxx			= Nomes internos 
--								: [ENT para [xxx			= Tabelas internas
--								: 'ENT' para 'xxx'		= Referência ao docto NFS
--								: ENTCAN para xxxCAN	= Referências ao documento Cancelamento
--								: ENTDEV para xxxDEV	= Referências ao documento Devolução
--								: [ODLN para [xxxx		= Tabelas B1 docto marketing - cabec
--								: [DLN para [xxx			= Tabelas B1 docto marketing - linhas
--
--	Substituições Manuais		: t1.[BaseType] = (adequar para cada FVT)
--
-------------------------------------------------------------------------------------------------------

if OBJECT_ID ('FVT_ENT_5') is not null
	drop function FVT_ENT_5
go
create function FVT_ENT_5 
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime, 
...
(
select	
		case when t0.CANCELED <> 'C' then 'ENT' else 'ENTCAN' end  as 'Tipo DOC',
		...
		(Select SUM(isnull(t10.LineTotal,0)) From [DLN3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [DLN13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		isnull((select SUM(isnull(t10.LineTotal,0)) from [DLN2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
inner join [DLN12] as t2 on t2.DocEntry = t0.DocEntry
...
from [DLN1] as t1
inner join [ENT] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [DLN4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		'Código da Despesa' as 'Código da Despesa' from [DLN1] as t0 
inner join [DLN5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [ENT_IRF_SUM] as t0 group by t0.DocEntry2, t0.[Linha Doc2]
),

[ENT_FINAL] as 
(
select (case when t0.[Tipo DOC] = 'ENTCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*  
from [ENT] as t0 
left outer join [ENT_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [ENT_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [ENT_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
)
...
from [ENT_FINAL] as t0 
left outer join [FVT_ENTDEV_5] (@BPLIdIni, @BPLIdFim, @DataIni, @DataFim,'-CAN') as t1 
		on (t0.DocEntry = t1.DocEntryBase and t0.[Linha Doc] = t1.BaseLine and t1.[BaseType] = '15' and t1.[Tipo DOC]='ENTCAN' and @CAN = '+CAN')		
left outer join [ENT_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '15' and t2.[Tipo DOC]='ENTCAN' and @CAN = '+CAN')		
where (t0.[Tipo DOC] ='ENT' and @CAN = '+CAN') or @CAN = '-CAN'
)
