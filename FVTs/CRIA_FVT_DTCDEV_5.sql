-- CRIA_FVT_DTCDEV_5 ------------------------------------------------------------------------------------------------------------------------------
--
-- @CAN = '+CAN' TRAZ AS DEVOLUÇÕES CANCELADAS E SOMA ÀS DEVOLUÇÕES. NÃO MOSTRA AS LINHAS DAS DTCDEVCAN (Tipo DOC), MAS TRAZ AS [Tipo DOC] = DTCCAN 
-- @CAN ='-CAN' TRAZ A DEVULUÇÃO ORIGINAL, O CANCELAMENTO DELA, MAS NÃO TRAZ OS CANCELAMENTOS DAS NFS (Tipo DOC] = 'DTCCAN'
--
--	Substituições				: [DTC para [xxx									= Tabelas internas
--								: DTCCAN para xxxCAN								= Referências ao documento Cancelamento
--								: DTCDEV para xxxDEV								= Referências ao documento Devolução
--								: [ORIN para [xxxx									= Tabelas B1 docto marketing - cabec
--								: [RIN para [xxx									= Tabelas B1 docto marketing - linhas
--								: t2.[BaseType] = '14' para t2.[BaseType] = 'xx'	= Alterar para o Documento xx
--
--------------------------------------------------------------------------------------------------------------------------------------------------
if OBJECT_ID ('FVT_DTCDEV_5') is not null
	drop function FVT_DTCDEV_5
go
create function FVT_DTCDEV_5
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime,
...
--declare @BPLIdIni as int = 0, 
--		@BPLIdFim as int = 99, 
--		@DataIni as datetime = '20140801', 
--		@DataFim as datetime = '20140831',
--		@CAN as char(4) = '+CAN'	;
-------------------------------------------------------------------------------------------------------		
With 
[PLANO_CTAS] as
...
select	t0.AcctCode as 'Conta Chave',
				(case when (select t9.EnbSgmnAct from [CINF] as t9) = 'Y' then (t0.Segment_0) else (t0.AcctCode) end) as 'Conta Contábil',
...
(
select	
		case when t0.CANCELED = 'C'	then 'DTCDEVCAN'
			 when t0.SeqCode = 1 then 'DTCCAN'	else 'DTCDEV' end as 'Tipo DOC',
		...
		(select t9.CardFName from [OCRD] as t9 where t9.CardCode = t2.Carrier) as 'Nome Transportadora',
		isnull(t2.QoP,0) as 'Quantidade Embalagem',
		t2.PackDesc as 'Descrição Embalagem',
		(-1)*isnull(t2.GrsWeight,0) as	'Peso Bruto',
		(-1)*isnull(t2.NetWeight,0) as	'Peso Líquido',
		...
		(-1)*((Select SUM(isnull(t10.LineTotal,0)) From [RIN3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [RIN13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		( SUM (case when t1.TaxOnly = 'Y' then 0.00 else (-1)*(t1.LineTotal)*(case when (t0.DocTotal + t0.DiscSum) = 0.00 then 1.00 else ((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) end) end ) over (partition by t0.docentry) ) as 'Total Mercadoria R$ Doc', /* Total Mercadoria R$ Documento */ 
		------------------------------------------- Detalhe do Item ---------------------------------------------------------------------------------------------------
		...
		(case when t1.TaxOnly = 'Y' then 0.00 else (-1)*t1.Quantity end) as 'Quantidade',
		...
		(-1)*t1.Price as 'Unitário Moeda',
		t1.Currency as 'Moeda',
		t1.Rate as 'Taxa Moeda',
		(case when t1.TaxOnly = 'Y' then 0.00 else (case when t1.Rate <> 0	then ((-1)*t1.Price * t1.Rate) else (-1)*t1.Price end ) end) as 'Unitário R$',
		(case when t1.TaxOnly = 'Y' then 0.00 else (-1)*(t1.LineTotal)*(case when (t0.DocTotal + t0.DiscSum) = 0.00 then 1.00 else ((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) end) end ) as 'Total Mercadoria R$',
		(case when t1.TaxOnly = 'Y' then 0.00 else (case when t1.Rate <> 0	then (-1)*(t1.TotalFrgn)*(case when (t0.DocTotal + t0.DiscSum) = 0.00 then 1.00 else ((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) end) 
																			else (-1)*(t1.LineTotal)*(case when (t0.DocTotal + t0.DiscSum) = 0.00 then 1.00 else ((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) end) end ) end ) as 'Total Mercadoria Moeda',
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RIN2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RIN13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		(-1)*(t1.StockPrice * t1.Quantity) as 'CMV',
		t1.BaseEntry as 'DocEntryBase',
		t1.BaseLine as 'BaseLine',
		t1.BaseType as 'BaseType',
		(case	when t1.basetype = 20 then (select t9.SeqCode from OPDN as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 18 then (select t9.SeqCode from OPCH as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 15 then (select t9.SeqCode from ODLN as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 13 then (select t9.SeqCode from OINV as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 23 then (select t9.SeqCode from OQUT as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 17 then (select t9.SeqCode from ORDR as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 16 then (select t9.SeqCode from ORDN as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 14 then (select t9.SeqCode from ORIN as t9 where t9.DocEntry = t1.BaseEntry)				
					when t1.basetype = 540000006 then (select t9.SeqCode from OPQT as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 22 then (select t9.SeqCode from OPOR as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 21 then (select t9.SeqCode from ORPD as t9 where t9.DocEntry = t1.BaseEntry)
					when t1.basetype = 19 then (select t9.SeqCode from ORPC as t9 where t9.DocEntry = t1.BaseEntry)				
...
inner join [RIN12] as t2 on t2.DocEntry = t0.DocEntry
...
left outer join [RIN1] as t10 on t10.DocEntry = t1.BaseEntry and t10.LineNum = t1.BaseLine and t1.BaseType = 14   /* liga ao doc original do cancelamento da devolução) */
where	(t0.DocDate between @DataIni and @DataFim) 
		and ( t0.BPLId is null or t0.BPLId between @BPLIdIni and @BPLIdFim)
		and ( (isnull(t1.BaseType,0) = 203 or isnull(t10.BaseType,0) = 203) ) 
),
...
select	t1.DocEntry as 'DocEntry1',
				...
				isnull((-1)*t4.TaxSum,0) as 'Valor Imposto',
				isnull((-1)*t4.BaseSum,0) as 'Base Imposto',
...
from [RIN1] as t1
inner join [DTCDEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [RIN4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		sum((-1)*t1.WTAmnt) as 'Valor Imposto',
		sum((-1)*t1.TaxbleAmnt) as 'Base Imposto',		
		...
		'Código da Despesa' as 'Código da Despesa' 
from [DTCDEV] as t2 		
inner join [RIN1] as t0 on t0.DocEntry = t2.DocEntry and t0.LineNum = t2.[Linha Doc]  
inner join [RIN5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [DTCDEV_IRF_SUM] as t0 
group by t0.DocEntry2, t0.[Linha Doc2]
),

[DTCDEV_ADTO] as
(
select	t0.DocEntry,
		sum(t1.DrawnSum) as 'Valor Adto Usado' 
from [ORIN] as t0 
inner join [RIN9] as t1 on t0.DocEntry = t1.DocEntry
Group by t0.DocEntry
),

[DTCDEV_FINAL] as 
(
select	(case when t0.[Tipo DOC] = 'DTCDEVCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*
from [DTCDEV] as t0 
left outer join [DTCDEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [DTCDEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [DTCDEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [DTCDEV_ADTO] as t4 on t4.Docentry = t0.DocEntry
)
...
(select	
		t0.[Tipo DOC], 
		t0.[Status Docto],
		(case when t2.[Tipo DOC] is not null then t2.[Tipo DOC] else '' end)  as 'Cancelada', 
		t0.[Código PN], 
		t0.[Nome Abreviado], 
		t0.[Nome PN], 
		t0.[Grupo PN],
		...
		t0.[S]*isnull(t0.[DA Não Distr Frete],0) + isnull(t2.[S]*t2.[DA Não Distr Frete],0) as 'DA Não Distr Frete',
		t0.[S]*isnull(t0.[DA Não Distr Seguro],0) + isnull(t2.[S]*t2.[DA Não Distr Seguro],0) as 'DA Não Distr Seguro',
		t0.[S]*isnull(t0.[DA Não Distr Outros],0) + isnull(t2.[S]*t2.[DA Não Distr Outros],0) as 'DA Não Distr Outros',
		t0.[Tipo de Envio] as 'Tipo de Envio',
		t0.[Uso Principal],
		t0.[S]*isnull(t0.[Valor Adto Usado],0) + isnull(t2.[S]*t2.[Valor Adto Usado],0) as 'Valor Adto Usado',
		----------------------------------------- Detalhe do Item ---------------------------------------------------------------------------------------------------
		t0.[Status Linha],
		t0.[Código Item], 
		t0.[Descrição Item], 
		...
		t0.[Conta CPV], 
		t0.[Nome Conta CPV], 
		(t0.[S]*t0.Quantidade) + isnull(t2.[S]*t2.Quantidade,0) as 'Quantidade',
		(t0.[S]*t0.[Quantidade Entregue]) + isnull(t2.[S]*t2.[Quantidade Entregue],0) as 'Quantidade Entregue',
		t0.UMV, 
		t0.ItensUMV, 
		t0.UME,
		t0.[Moeda],
		(t0.[S]*t0.[Unitário Moeda]) + isnull(t2.[S]*t2.[Unitário Moeda],0)  as 'Unitário Moeda',
		t0.[Taxa Moeda],
		(t0.[S]*t0.[Total Mercadoria Moeda]) + isnull(t2.[S]*t2.[Total Mercadoria Moeda],0) as 'Total Mercadoria Moeda',			
		(t0.[S]*t0.[Unitário R$]) + isnull(t2.[S]*t2.[Unitário R$],0) as 'Unitário R$',
		(t0.[S]*t0.CMV) + isnull(t2.[S]*t2.CMV,0) as 'CMV',
		(t0.[S]*t0.[Total Mercadoria R$]) + isnull(t2.[S]*t2.[Total Mercadoria R$],0) as 'Total Mercadoria R$',
		(t0.[S]*(t0.[DA Frete]) + isnull(t2.[S]*(t2.[DA Frete]),0)) as 'DA Frete',
		(t0.[S]*(t0.[DA Seguro]) + isnull(t2.[S]*(t2.[DA Seguro]),0)) as 'DA Seguro',
		(t0.[S]*(t0.[DA Outros]) + isnull(t2.[S]*(t2.[DA Outros]),0)) as 'DA Outros',
		t0.[IMPOSTOS-I],
		t0.[IMPOSTOS-NI],	
		...
		--		- round(sum (isnull(t2.[IRF-TOTAL],0)) over (partition by t2.DocEntry),2) ),0), 2)
		...
		(t0.[S]*isnull(t0.[COFINS-I],0)) + isnull(t2.[S]*t2.[COFINS-I],0) as 'COFINS-I',
		(t0.[S]*isnull(t0.[PIS-I],0)) + isnull(t2.[S]*t2.[PIS-I],0) as 'PIS-I',
		(t0.[S]*isnull(t0.[ICMS-I],0)) + isnull(t2.[S]*t2.[ICMS-I],0) as 'ICMS-I',
		(t0.[S]*isnull(t0.[ICMS-ST-I],0)) + isnull(t2.[S]*t2.[ICMS-ST-I],0) as 'ICMS-ST-I',
		(t0.[S]*isnull(t0.[IPI-I],0)) + isnull(t2.[S]*t2.[IPI-I],0) as 'IPI-I',
		(t0.[S]*isnull(t0.[ISS-I],0)) + isnull(t2.[S]*t2.[ISS-I],0) as 'ISS-I',
		(t0.[S]*isnull(t0.[CSLL-I],0)) + isnull(t2.[S]*t2.[CSLL-I],0) as 'CSLL-I',
		(t0.[S]*isnull(t0.[IRPJ-I],0)) + isnull(t2.[S]*t2.[IRPJ-I],0) as 'IRPJ-I',
		(t0.[S]*isnull(t0.[II-I],0)) + isnull(t2.[S]*t2.[II-I],0) as 'II-I',
		(t0.[S]*isnull(t0.[OUTROS IMP-I],0)) + isnull(t2.[S]*t2.[OUTROS IMP-I],0) as 'OUTROS IMP-I',

		(t0.[S]*isnull(t0.[COFINS-NI],0)) + isnull(t2.[S]*t2.[COFINS-NI],0) as 'COFINS-NI',
		(t0.[S]*isnull(t0.[PIS-NI],0)) + isnull(t2.[S]*t2.[PIS-NI],0) as 'PIS-NI',
		(t0.[S]*isnull(t0.[ICMS-NI],0)) + isnull(t2.[S]*t2.[ICMS-NI],0) as 'ICMS-NI',
		(t0.[S]*isnull(t0.[ICMS-ST-NI],0)) + isnull(t2.[S]*t2.[ICMS-ST-NI],0) as 'ICMS-ST-NI',
		(t0.[S]*isnull(t0.[IPI-NI],0)) + isnull(t2.[S]*t2.[IPI-NI],0) as 'IPI-NI',
		(t0.[S]*isnull(t0.[ISS-NI],0)) + isnull(t2.[S]*t2.[ISS-NI],0) as 'ISS-NI',
		(t0.[S]*isnull(t0.[CSLL-NI],0)) + isnull(t2.[S]*t2.[CSLL-NI],0) as 'CSLL-NI',
		(t0.[S]*isnull(t0.[IRPJ-NI],0)) + isnull(t2.[S]*t2.[IRPJ-NI],0) as 'IRPJ-NI',
		(t0.[S]*isnull(t0.[II-NI],0)) + isnull(t2.[S]*t2.[II-NI],0) as 'II-NI',
		(t0.[S]*isnull(t0.[OUTROS IMP-NI],0)) + isnull(t2.[S]*t2.[OUTROS IMP-NI],0) as 'OUTROS IMP-NI',

		(t0.[S]*isnull(t0.[Base COFINS],0)) + isnull(t2.[S]*t2.[Base COFINS],0) as 'Base COFINS',
		(t0.[S]*isnull(t0.[Base PIS],0)) + isnull(t2.[S]*t2.[Base PIS],0) as 'Base PIS',
		(t0.[S]*isnull(t0.[Base ICMS],0)) + isnull(t2.[S]*t2.[Base ICMS],0) as 'Base ICMS',
		(t0.[S]*isnull(t0.[Base ICMS-ST],0)) + isnull(t2.[S]*t2.[Base ICMS-ST],0) as 'Base ICMS-ST',
		(t0.[S]*isnull(t0.[Base IPI],0)) + isnull(t2.[S]*t2.[Base IPI],0) as 'Base IPI',
		(t0.[S]*isnull(t0.[Base ISS],0)) + isnull(t2.[S]*t2.[Base ISS],0) as 'Base ISS',
		(t0.[S]*isnull(t0.[Base CSLL],0)) + isnull(t2.[S]*t2.[Base CSLL],0) as 'Base CSLL',
		(t0.[S]*isnull(t0.[Base IRPJ],0)) + isnull(t2.[S]*t2.[Base IRPJ],0) as 'Base IRPJ',
		(t0.[S]*isnull(t0.[Base II],0)) + isnull(t2.[S]*t2.[Base II],0) as 'Base II',
		(t0.[S]*isnull(t0.[Base OUTROS IMP],0)) + isnull(t2.[S]*t2.[Base OUTROS IMP],0) as 'Base OUTROS IMP',

		(t0.[S]*isnull(t0.[DA COFINS-I],0)) + isnull(t2.[S]*t2.[DA COFINS-I],0) as 'DA COFINS-I',
		(t0.[S]*isnull(t0.[DA PIS-I],0)) + isnull(t2.[S]*t2.[DA PIS-I],0) as 'DA PIS-I',
		(t0.[S]*isnull(t0.[DA ICMS-I],0)) + isnull(t2.[S]*t2.[DA ICMS-I],0) as 'DA ICMS-I',
		(t0.[S]*isnull(t0.[DA ICMS-ST-I],0)) + isnull(t2.[S]*t2.[DA ICMS-ST-I],0) as 'DA ICMS-ST-I',
		(t0.[S]*isnull(t0.[DA IPI-I],0)) + isnull(t2.[S]*t2.[DA IPI-I],0) as 'DA IPI-I',
		(t0.[S]*isnull(t0.[DA ISS-I],0)) + isnull(t2.[S]*t2.[DA ISS-I],0) as 'DA ISS-I',
		(t0.[S]*isnull(t0.[DA CSLL-I],0)) + isnull(t2.[S]*t2.[DA CSLL-I],0) as 'DA CSLL-I',
		(t0.[S]*isnull(t0.[DA IRPJ-I],0)) + isnull(t2.[S]*t2.[DA IRPJ-I],0) as 'DA IRPJ-I',
		(t0.[S]*isnull(t0.[DA II-I],0)) + isnull(t2.[S]*t2.[DA II-I],0) as 'DA II-I',
		(t0.[S]*isnull(t0.[DA OUTROS IMP-I],0)) + isnull(t2.[S]*t2.[DA OUTROS IMP-I],0) as 'DA OUTROS IMP-I',

		(t0.[S]*isnull(t0.[DA COFINS-NI],0)) + isnull(t2.[S]*t2.[DA COFINS-NI],0) as 'DA COFINS-NI',
		(t0.[S]*isnull(t0.[DA PIS-NI],0)) + isnull(t2.[S]*t2.[DA PIS-NI],0) as 'DA PIS-NI',
		(t0.[S]*isnull(t0.[DA ICMS-NI],0)) + isnull(t2.[S]*t2.[DA ICMS-NI],0) as 'DA ICMS-NI',
		(t0.[S]*isnull(t0.[DA ICMS-ST-NI],0)) + isnull(t2.[S]*t2.[DA ICMS-ST-NI],0) as 'DA ICMS-ST-NI',
		(t0.[S]*isnull(t0.[DA IPI-NI],0)) + isnull(t2.[S]*t2.[DA IPI-NI],0) as 'DA IPI-NI',
		(t0.[S]*isnull(t0.[DA ISS-NI],0)) + isnull(t2.[S]*t2.[DA ISS-NI],0) as 'DA ISS-NI',
		(t0.[S]*isnull(t0.[DA CSLL-NI],0)) + isnull(t2.[S]*t2.[DA CSLL-NI],0) as 'DA CSLL-NI',
		(t0.[S]*isnull(t0.[DA IRPJ-NI],0)) + isnull(t2.[S]*t2.[DA IRPJ-NI],0) as 'DA IRPJ-NI',
		(t0.[S]*isnull(t0.[DA II-NI],0)) + isnull(t2.[S]*t2.[DA II-NI],0) as 'DA II-NI',
		(t0.[S]*isnull(t0.[DA OUTROS IMP-NI],0)) + isnull(t2.[S]*t2.[DA OUTROS IMP-NI],0) as 'DA OUTROS IMP-NI',

		(t0.[S]*isnull(t0.[DA Base COFINS],0)) + isnull(t2.[S]*t2.[DA Base COFINS],0) as 'DA Base COFINS',
		(t0.[S]*isnull(t0.[DA Base PIS],0)) + isnull(t2.[S]*t2.[DA Base PIS],0) as 'DA Base PIS',
		(t0.[S]*isnull(t0.[DA Base ICMS],0)) + isnull(t2.[S]*t2.[DA Base ICMS],0) as 'DA Base ICMS',
		(t0.[S]*isnull(t0.[DA Base ICMS-ST],0)) + isnull(t2.[S]*t2.[DA Base ICMS-ST],0) as 'DA Base ICMS-ST',
		(t0.[S]*isnull(t0.[DA Base IPI],0)) + isnull(t2.[S]*t2.[DA Base IPI],0) as 'DA Base IPI',
		(t0.[S]*isnull(t0.[DA Base ISS],0)) + isnull(t2.[S]*t2.[DA Base ISS],0) as 'DA Base ISS',
		(t0.[S]*isnull(t0.[DA Base CSLL],0)) + isnull(t2.[S]*t2.[DA Base CSLL],0) as 'DA Base CSLL',
		(t0.[S]*isnull(t0.[DA Base IRPJ],0)) + isnull(t2.[S]*t2.[DA Base IRPJ],0) as 'DA Base IRPJ',
		(t0.[S]*isnull(t0.[DA Base II],0)) + isnull(t2.[S]*t2.[DA Base II],0) as 'DA Base II',
		(t0.[S]*isnull(t0.[DA Base OUTROS IMP],0)) + isnull(t2.[S]*t2.[DA Base OUTROS IMP],0) as 'DA Base OUTROS IMP',	
			
		(t0.[S]*isnull(t0.[IRF-ISSF],0)) + isnull(t2.[S]*t2.[IRF-ISSF],0) as 'IRF-ISSF',
		(t0.[S]*isnull(t0.[IRF-IRRF],0)) + isnull(t2.[S]*t2.[IRF-IRRF],0) as 'IRF-IRRF',
		(t0.[S]*isnull(t0.[IRF-PIS],0)) + isnull(t2.[S]*t2.[IRF-PIS],0) as 'IRF-PIS',
		(t0.[S]*isnull(t0.[IRF-COFINS],0)) + isnull(t2.[S]*t2.[IRF-COFINS],0) as 'IRF-COFINS',
		(t0.[S]*isnull(t0.[IRF-CSLL],0)) + isnull(t2.[S]*t2.[IRF-CSLL],0) as 'IRF-CSLL',			
		(t0.[S]*isnull(t0.[IRF-INSS],0)) + isnull(t2.[S]*t2.[IRF-INSS],0) as 'IRF-INSS',
		(t0.[S]*isnull(t0.[IRF-10833],0)) + isnull(t2.[S]*t2.[IRF-10833],0) as 'IRF-10833',
		(t0.[S]*isnull(t0.[IRF-OUTROS],0)) + isnull(t2.[S]*t2.[IRF-OUTROS],0) as 'IRF-OUTROS',
		(t0.[S]*isnull(t0.[IRF-TOTAL],0)) + isnull(t2.[S]*t2.[IRF-TOTAL],0) as 'IRF-TOTAL',
		(t0.[S]*isnull(t0.[Base IRF-ISSF],0)) + isnull(t2.[S]*t2.[Base IRF-ISSF],0) as 'Base IRF-ISSF',
		(t0.[S]*isnull(t0.[Base IRF-IRRF],0)) + isnull(t2.[S]*t2.[Base IRF-IRRF],0) as 'Base IRF-IRRF',
		(t0.[S]*isnull(t0.[Base IRF-PIS],0)) + isnull(t2.[S]*t2.[Base IRF-PIS],0) as 'Base IRF-PIS',
		(t0.[S]*isnull(t0.[Base IRF-COFINS],0)) + isnull(t2.[S]*t2.[Base IRF-COFINS],0) as 'Base IRF-COFINS',
		(t0.[S]*isnull(t0.[Base IRF-CSLL],0)) + isnull(t2.[S]*t2.[Base IRF-CSLL],0) as 'Base IRF-CSLL',			
		(t0.[S]*isnull(t0.[Base IRF-INSS],0)) + isnull(t2.[S]*t2.[Base IRF-INSS],0) as 'Base IRF-INSS',
		(t0.[S]*isnull(t0.[Base IRF-10833],0)) + isnull(t2.[S]*t2.[Base IRF-10833],0) as 'Base IRF-10833',
		(t0.[S]*isnull(t0.[Base IRF-OUTROS],0)) + isnull(t2.[S]*t2.[Base IRF-OUTROS],0) as 'Base IRF-OUTROS',

		(t0.[S]*isnull(t0.[IRF-ISSF-P],0)) + isnull(t2.[S]*t2.[IRF-ISSF-P],0) as 'IRF-ISSF-P',
		(t0.[S]*isnull(t0.[IRF-IRRF-P],0)) + isnull(t2.[S]*t2.[IRF-IRRF-P],0) as 'IRF-IRRF-P',
		(t0.[S]*isnull(t0.[IRF-PIS-P],0)) + isnull(t2.[S]*t2.[IRF-PIS-P],0) as 'IRF-PIS-P',
		(t0.[S]*isnull(t0.[IRF-COFINS-P],0)) + isnull(t2.[S]*t2.[IRF-COFINS-P],0) as 'IRF-COFINS-P',
		(t0.[S]*isnull(t0.[IRF-CSLL-P],0)) + isnull(t2.[S]*t2.[IRF-CSLL-P],0) as 'IRF-CSLL-P',			
		(t0.[S]*isnull(t0.[IRF-INSS-P],0)) + isnull(t2.[S]*t2.[IRF-INSS-P],0) as 'IRF-INSS-P',
		(t0.[S]*isnull(t0.[IRF-10833-P],0)) + isnull(t2.[S]*t2.[IRF-10833-P],0) as 'IRF-10833-P',
		(t0.[S]*isnull(t0.[IRF-OUTROS-P],0)) + isnull(t2.[S]*t2.[IRF-OUTROS-P],0) as 'IRF-OUTROS-P',
		(t0.[S]*isnull(t0.[IRF-TOTAL-P],0)) + isnull(t2.[S]*t2.[IRF-TOTAL-P],0) as 'IRF-TOTAL-P',
		(t0.[S]*isnull(t0.[Base IRF-ISSF-P],0)) + isnull(t2.[S]*t2.[Base IRF-ISSF-P],0) as 'Base IRF-ISSF-P',
		(t0.[S]*isnull(t0.[Base IRF-IRRF-P],0)) + isnull(t2.[S]*t2.[Base IRF-IRRF-P],0) as 'Base IRF-IRRF-P',
		(t0.[S]*isnull(t0.[Base IRF-PIS-P],0)) + isnull(t2.[S]*t2.[Base IRF-PIS-P],0) as 'Base IRF-PIS-P',
		(t0.[S]*isnull(t0.[Base IRF-COFINS-P],0)) + isnull(t2.[S]*t2.[Base IRF-COFINS-P],0) as 'Base IRF-COFINS-P',
		(t0.[S]*isnull(t0.[Base IRF-CSLL-P],0)) + isnull(t2.[S]*t2.[Base IRF-CSLL-P],0) as 'Base IRF-CSLL-P',			
		(t0.[S]*isnull(t0.[Base IRF-INSS-P],0)) + isnull(t2.[S]*t2.[Base IRF-INSS-P],0) as 'Base IRF-INSS-P',
		(t0.[S]*isnull(t0.[Base IRF-10833-P],0)) + isnull(t2.[S]*t2.[Base IRF-10833-P],0) as 'Base IRF-10833-P',
		(t0.[S]*isnull(t0.[Base IRF-OUTROS-P],0)) + isnull(t2.[S]*t2.[Base IRF-OUTROS-P],0) as 'Base IRF-OUTROS-P',
...
from [DTCDEV_FINAL] as t0 
left outer join [DTCDEV_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '14' and t2.[Tipo DOC]='DTCDEVCAN' and @CAN = '+CAN')	
where	( t0.[Tipo DOC] = 'DTCDEV' and @CAN = '+CAN' )
		or @CAN = '-CAN'   
