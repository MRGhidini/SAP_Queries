...
--
--	Substituições				: _PEC para _xxx			= Nomes internos 
--								: [PEC para [xxx			= Tabelas internas
--								: 'PEC' para 'xxx'			= Referência ao docto NFS
--								: PECCAN para xxxCAN		= Referências ao documento Cancelamento
--								: PECDEV para xxxDEV		= Referências ao documento Devolução
--								: [OPOR para [xxxx			= Tabelas B1 docto marketing - cabec
--								: [POR para [xxx			= Tabelas B1 docto marketing - linhas
--
-------------------------------------------------------------------------------------------------------

if OBJECT_ID ('FVT_PEC_5') is not null
	drop function FVT_PEC_5
go
create function FVT_PEC_5 
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime
...
select	
		...
		(case when isnull(t0.CANCELED,'N') <> 'Y' then '' else 'PECCAN' end)  as 'Cancelada',
		...
		(Select SUM(isnull(t10.LineTotal,0)) From [POR3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [POR13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		isnull((select SUM(isnull(t10.LineTotal,0)) from [POR2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
left outer join [POR12] as t2 on t2.DocEntry = t0.DocEntry
...
from [POR1] as t1
inner join [PEC] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [POR4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		'Código da Despesa' as 'Código da Despesa' from [POR1] as t0 
inner join [POR5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [PEC_IRF_SUM] as t0 group by t0.DocEntry2, t0.[Linha Doc2]
),
...
select 	t0.*,
		...
		t2.*    
from [PEC] as t0 
left outer join [PEC_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [PEC_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]
left outer join [PEC_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]	
)
--
