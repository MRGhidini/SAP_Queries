-- CRIA_FVT_NFSDEV_5 ------------------------------------------------------------------------------------------------------------------------------
--
-- @CAN = '+CAN' TRAZ AS DEVOLUÇÕES CANCELADAS E SOMA ÀS DEVOLUÇÕES. NÃO MOSTRA AS LINHAS DAS NFSDEVCAN (Tipo DOC), MAS TRAZ AS [Tipo DOC] = NFSCAN 
-- @CAN ='-CAN' TRAZ A DEVULUÇÃO ORIGINAL, O CANCELAMENTO DELA, MAS NÃO TRAZ OS CANCELAMENTOS DAS NFS (Tipo DOC] = 'NFSCAN'
--
--	Substituições				: [NFS para [xxx									= Tabelas internas
--								: NFSCAN para xxxCAN								= Referências ao documento Cancelamento
--								: NFSDEV para xxxDEV								= Referências ao documento Devolução
...
--
--------------------------------------------------------------------------------------------------------------------------------------------------
if OBJECT_ID ('FVT_NFSDEV_5') is not null
	drop function FVT_NFSDEV_5
go
create function FVT_NFSDEV_5
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime,
...
(
select	
		case when t0.CANCELED = 'C'	then 'NFSDEVCAN'
			 when t0.SeqCode = 1 then 'NFSCAN'	else 'NFSDEV' end as 'Tipo DOC',
...
where	(t0.DocDate between @DataIni and @DataFim) 
		...
		and ( not(isnull(t1.BaseType,0) = 203 or isnull(t10.BaseType,0) = 203) ) 
),
...
from [RIN1] as t1
inner join [NFSDEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
...
(
select	t0.DocEntry,
		(-1)*sum(t1.DrawnSum) as 'Valor Adto Usado' 
from [ORIN] as t0 
...
[NFSDEV_FINAL] as 
(
select	(case when t0.[Tipo DOC] = 'NFSDEVCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*
from [NFSDEV] as t0 
left outer join [NFSDEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [NFSDEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [NFSDEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [NFSDEV_ADTO] as t4 on t4.Docentry = t0.DocEntry
)
...
from [NFSDEV_FINAL] as t0 
left outer join [NFSDEV_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '14' and t2.[Tipo DOC]='NFSDEVCAN' and @CAN = '+CAN')	
where	( t0.[Tipo DOC] = 'NFSDEV' and @CAN = '+CAN' )
		or @CAN = '-CAN'   
