-- CRIA_FVT_ENTDEV_5 ---------------------------------------------------------
--
-- @CAN = '+CAN' TRAZ AS DEVOLUÇÕES CANCELADAS E SOMA ÀS DEVOLUÇÕES. NÃO MOSTRA AS LINHAS DAS ENTDEVCAN (Tipo DOC), MAS TRAZ AS [Tipo DOC] = ENTCAN 
-- @CAN ='-CAN' TRAZ A DEVULUÇÃO ORIGINAL, O CANCELAMENTO DELA, MAS NÃO TRAZ OS CANCELAMENTOS DAS NFS (Tipo DOC] = 'ENTCAN'
--
--	Substituições				: [ENT para [xxx			= Tabelas internas
--								: ENTCAN para xxxCAN		= Referências ao documento Cancelamento
--								: ENTDEV para xxxDEV		= Referências ao documento Devolução
--								: [ORDN para [xxxx			= Tabelas B1 docto marketing - cabec
--								: [RDN para [xxx			= Tabelas B1 docto marketing - linhas
--
--	Substituições Manuais		: t1.[BaseType] = (adequar para cada FVT)
--
-------------------------------------------------------------------------------------------------------
if OBJECT_ID ('FVT_ENTDEV_5') is not null
	drop function FVT_ENTDEV_5
go
create function FVT_ENTDEV_5
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime,
...
(
select	
		case when t0.CANCELED = 'C'	then 'ENTDEVCAN'
			 when t0.SeqCode = 1 then 'ENTCAN'	else 'ENTDEV' end as 'Tipo DOC',
		...
		(-1)*((Select SUM(isnull(t10.LineTotal,0)) From [RDN3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [RDN13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RDN2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RDN13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
inner join [RDN12] as t2 on t2.DocEntry = t0.DocEntry
...
--inner join OWHS as t9 on t9.WhsCode = t1.WhsCode and t9.BPLid between @BPLIdIni and @BPLIdFim
...
from [RDN1] as t1
inner join [ENTDEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [RDN4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
from [ENTDEV] as t2 		
inner join [RDN1] as t0 on t0.DocEntry = t2.DocEntry and t0.LineNum = t2.[Linha Doc]  
inner join [RDN5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [ENTDEV_IRF_SUM] as t0 
group by t0.DocEntry2, t0.[Linha Doc2]
),

[ENTDEV_FINAL] as 
(
select	(case when t0.[Tipo DOC] = 'ENTDEVCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*
from [ENTDEV] as t0 
left outer join [ENTDEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [ENTDEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [ENTDEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
)
...
from [ENTDEV_FINAL] as t0 
left outer join [ENTDEV_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '16' and t2.[Tipo DOC]='ENTDEVCAN' and @CAN = '+CAN')	
where	( t0.[Tipo DOC] = 'ENTDEV' and @CAN = '+CAN' )
		or @CAN = '-CAN'   
