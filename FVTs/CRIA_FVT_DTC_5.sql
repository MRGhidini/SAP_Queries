...
--
--	Substituições(CTRL H)		: _DTC para _xxx			= Nomes internos 
--								: [DTC para [xxx			= Tabelas internas
--								: 'DTC' para 'xxx'		= Referência ao docto NFS
--								: DTCCAN para xxxCAN	= Referências ao documento Cancelamento
--								: DTCDEV para xxxDEV	= Referências ao documento Devolução
--								: [ODPI para [xxxx		= Tabelas B1 docto marketing - cabec
--								: [DPI para [xxx			= Tabelas B1 docto marketing - linhas
--
--	Substituições Manuais		: t1.[BaseType] = (adequar para cada FVT)
--
-------------------------------------------------------------------------------------------------------

if OBJECT_ID ('FVT_DTC_5') is not null
	drop function FVT_DTC_5
go
create function FVT_DTC_5 
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime, 
...
------------------------------------------------------------------------------------------------------	
	...
	--	@BPLIdFim as int = 99, 
	--	@DataIni as datetime = '20140201', 
	--	@DataFim as datetime = '20140228', 
	--	@CAN as char(4) = '-CAN';
...
(
select	
		case when t0.CANCELED <> 'C' then 'DTC' else 'DTCCAN' end as 'Tipo DOC',
		...
		(Select SUM(isnull(t10.LineTotal,0)) From [DPI3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [DPI13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		--(t1.LineTotal)*((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) as 'Tot_Linha',
		--round((t1.LineTotal)*((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)),2) as 'Tot_Linha-round',
		--((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) as '%desc Tot_Linha',
		( SUM (case when t1.TaxOnly = 'Y' 
					then 0.00 
					else ((t1.LineTotal)*(case when (t0.DocTotal + t0.DiscSum) = 0.00 then 1.00 else ((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) end) ) end ) 
					over (partition by t0.docentry) ) as 'Total Mercadoria R$ Doc', /* Total Mercadoria R$ Documento */ 
		------------------------------------------- Detalhe do Item ---------------------------------------------------------------------------------------------------
		...
		isnull((select SUM(isnull(t10.LineTotal,0)) from [DPI2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		from [ODPI] as t0    
inner join [DPI1] as t1 on t1.DocEntry = t0.DocEntry
inner join [DPI12] as t2 on t2.DocEntry = t0.DocEntry
...
from [DPI1] as t1
inner join [DTC] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [DPI4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		'Código da Despesa' as 'Código da Despesa' from [DPI1] as t0 
inner join [DPI5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [DTC_IRF_SUM] as t0 group by t0.DocEntry2, t0.[Linha Doc2]
),
...
from [ODPI] as t0 
inner join [DPI9] as t1 on t0.DocEntry = t1.DocEntry
Group by t0.DocEntry
),

[DTC_FINAL] as 
(
select (case when t0.[Tipo DOC] = 'DTCCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*  
from [DTC] as t0 
left outer join [DTC_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [DTC_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]
left outer join [DTC_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [DTC_ADTO] as t4 on t4.Docentry = t0.DocEntry
)
...
(select	
		...
		t0.[S]*isnull(t0.[DA Não Distr Frete],0) + isnull(t2.[S]*t2.[DA Não Distr Frete],0) + isnull(t1.[DA Não Distr Frete],0) as 'DA Não Distr Frete',
		t0.[S]*isnull(t0.[DA Não Distr Seguro],0) + isnull(t2.[S]*t2.[DA Não Distr Seguro],0) + isnull(t1.[DA Não Distr Seguro],0) as 'DA Não Distr Seguro',
		t0.[S]*isnull(t0.[DA Não Distr Outros],0) + isnull(t2.[S]*t2.[DA Não Distr Outros],0) + isnull(t1.[DA Não Distr Outros],0) as 'DA Não Distr Outros',
		t0.[Tipo de Envio],
		t0.[Uso Principal],
		t0.[S]*isnull(t0.[Valor Adto Usado],0) + isnull(t2.[S]*t2.[Valor Adto Usado],0) + isnull(t1.[Valor Adto Usado],0) as 'Valor Adto Usado' ,
	------------------------------------------- Detalhe do Item ---------------------------------------------------------------------------------------------------
		t0.[Status Linha],
		t0.[Código Item], 
		t0.[Descrição Item], 
...
from [DTC_FINAL] as t0 
left outer join [FVT_DTCDEV_5] (@BPLIdIni, @BPLIdFim, @DataIni, @DataFim,'-CAN') as t1 
		on (t0.DocEntry = t1.DocEntryBase and t0.[Linha Doc] = t1.BaseLine and t1.[BaseType] = '13' and t1.[Tipo DOC]='DTCCAN' and @CAN = '+XXX')		
left outer join [DTC_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '203' and t2.[Tipo DOC]='DTCCAN' and @CAN = '+XXX')		
where  (t0.[Tipo DOC] ='DTC' and @CAN = '+CAN') or @CAN = '-CAN' 
)
