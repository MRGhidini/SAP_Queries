-- CRIA_FVT_NFEDEV_5 -------------------------------------------------------------------------------------------------------------------------------
--
-- @CAN = '+CAN' TRAZ AS DEVOLUÇÕES CANCELADAS E SOMA ÀS DEVOLUÇÕES. NÃO MOSTRA AS LINHAS DAS NFEDEVCAN (Tipo DOC), MAS TRAZ AS [Tipo DOC] = NFECAN 
-- @CAN ='-CAN' TRAZ A DEVULUÇÃO ORIGINAL, O CANCELAMENTO DELA, MAS NÃO TRAZ OS CANCELAMENTOS DAS NFS (Tipo DOC] = 'NFECAN'
--
--	Substituições				: [NFE para [xxx									= Tabelas internas
--								: NFECAN para xxxCAN								= Referências ao documento Cancelamento
--								: NFEDEV para xxxDEV								= Referências ao documento Devolução
--								: [ORPC para [xxxx									= Tabelas B1 docto marketing - cabec
--								: [RPC para [xxx									= Tabelas B1 docto marketing - linhas
--								: t2.[BaseType] = '19' para t2.[BaseType] = 'xx'	= Alterar para o Documento xx
--
---------------------------------------------------------------------------------------------------------------------------------------------------
if OBJECT_ID ('FVT_NFEDEV_5') is not null
	drop function FVT_NFEDEV_5
go
create function FVT_NFEDEV_5
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime,
...
(
select	
		case when t0.CANCELED = 'C'	then 'NFEDEVCAN'
			 when t0.SeqCode = 1 then 'NFECAN'	else 'NFEDEV' end as 'Tipo DOC',
...
where	--((isnull(t0.SeqCode,0) = 1 and @CAN = '+CAN') or (isnull(t0.SeqCode,0) <> 1)) and	
			(t0.DocDate between @DataIni and @DataFim) 
			and ( t0.BPLId is null or t0.BPLId between @BPLIdIni and @BPLIdFim)
			and ( not(isnull(t1.BaseType,0) = 204 or isnull(t10.BaseType,0) = 204) ) 
),
...
from [RPC1] as t1
inner join [NFEDEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
...
[NFEDEV_FINAL] as 
(
select	(case when t0.[Tipo DOC] = 'NFEDEVCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*
from [NFEDEV] as t0 
left outer join [NFEDEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [NFEDEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [NFEDEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [NFEDEV_ADTO] as t4 on t4.Docentry = t0.DocEntry
...
(select	
		...
		t0.[S]*isnull(t0.[DA Não Distr Frete],0) + t2.[S]*isnull(t2.[DA Não Distr Frete],0) as 'DA Não Distr Frete',
		t0.[S]*isnull(t0.[DA Não Distr Seguro],0) + t2.[S]*isnull(t2.[DA Não Distr Seguro],0) as 'DA Não Distr Seguro',
		t0.[S]*isnull(t0.[DA Não Distr Outros],0) + t2.[S]*isnull(t2.[DA Não Distr Outros],0) as 'DA Não Distr Outros',
...
from [NFEDEV_FINAL] as t0 
left outer join [NFEDEV_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '19' and t2.[Tipo DOC]='NFEDEVCAN' and @CAN = '+CAN')	
where	( t0.[Tipo DOC] = 'NFEDEV' and @CAN = '+CAN' )
		or @CAN = '-CAN'   
