...
--
--	Substituições				: _PEV para _xxx			= Nomes internos 
--								: [PEV para [xxx			= Tabelas internas
--								: 'PEV' para 'xxx'			= Referência ao docto NFS
--								: PEVCAN para xxxCAN		= Referências ao documento Cancelamento
--								: PEVDEV para xxxDEV		= Referências ao documento Devolução
--								: [ORDR para [xxxx			= Tabelas B1 docto marketing - cabec
--								: [RDR para [xxx			= Tabelas B1 docto marketing - linhas
--
-------------------------------------------------------------------------------------------------------

if OBJECT_ID ('FVT_PEV_5') is not null
	drop function FVT_PEV_5
go
create function FVT_PEV_5 
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime
...
select	
		...
		(case when isnull(t0.CANCELED,'N') <> 'Y' then '' else 'PEVCAN' end)  as 'Cancelada',
		...
		t0.AgentCode as 'Representante',
		isnull(oagp.AgentName,'') as 'Nome Representante',
		(Select SUM(isnull(t10.LineTotal,0)) From [RDR3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [RDR13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		case when isnull(t1.ShipDate,'') = '' then t0.DocDueDate else t1.ShipDate end as 'Data Entrega',
		...
		(select t9.InvntryUom from [OITM] as t9 where t9.ItemCode = t1.ItemCode) as 'UME',
		...
		isnull((select SUM(isnull(t10.LineTotal,0)) from [RDR2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
left outer join [RDR12] as t2 on t2.DocEntry = t0.DocEntry
left outer join [OCRD] as ocrd on ocrd.CardCode = t0.CardCode
left outer join [OITM] as oitm on oitm.ItemCode = t1.ItemCode
left outer join [OITB] as oitb on oitb.ItmsGrpCod = oitm.ItmsGrpCod
left outer join [OWHS] as owhs on owhs.WhsCode = t1.WhsCode
...
select	t1.DocEntry as 'DocEntry1',
				...
				isnull(round(t4.TaxSum,2),0) as 'Valor Imposto',
...
from [RDR1] as t1
inner join [PEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [RDR4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		'Código da Despesa' as 'Código da Despesa' from [RDR1] as t0 
inner join [RDR5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [PEV_IRF_SUM] as t0 group by t0.DocEntry2, t0.[Linha Doc2]
),
...
select 	t0.*,
		...
		t2.*    
from [PEV] as t0 
left outer join [PEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [PEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]
left outer join [PEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]	
)
--
-- DESTE PONTO EM DIANTE É DIFERENTE POR NÂO EXISTIR DEVOLUÇÃO
-- 
-------------------------------------------------------------------------------------------------------------------------				
	...
	(select	
			...
			t0.[Peso Bruto] as 'Peso Bruto',
			t0.[Peso Líquido] as 'Peso Líquido',
			t0.[Código Vendedor], 
			t0.[Nome Vendedor], 
			t0.Representante, 
			t0.[Nome Representante], 
			isnull(t0.[DA Não Distr Frete],0) as 'DA Não Distr Frete',
			isnull(t0.[DA Não Distr Seguro],0) as 'DA Não Distr Seguro',
			isnull(t0.[DA Não Distr Outros],0) as 'DA Não Distr Outros',
			t0.[Tipo de Envio],
			t0.[Uso Principal],
		...
		------------------------------------------- Detalhe do Item ---------------------------------------------------------------------------------------------------
			...
			t0.[Nome Conta CPV], 
			t0.Quantidade,
			t0.[Quantidade Entregue], 
			t0.UMV, 
			t0.ItensUMV, 
			t0.UME,
			t0.[Moeda],
			t0.[Unitário Moeda] as 'Unitário Moeda',
			t0.[Taxa Moeda],
			t0.[Total Mercadoria Moeda] as 'Total Mercadoria Moeda',			
			t0.[Unitário R$] as 'Unitário R$',
			t0.CMV as 'CMV',
			t0.[Total Mercadoria R$] as 'Total Mercadoria R$',
			((t0.[DA Linha Frete] + t0.[DA Distr Frete])) as 'DA Frete',
			((t0.[DA Linha Seguro] + t0.[DA Distr Seguro])) as 'DA Seguro',
			((t0.[DA Linha Outros] + t0.[DA Distr Outros])) as 'DA Outros',
			t0.[IMPOSTOS-I],
			t0.[IMPOSTOS-NI],
			( isnull(t0.[Total Mercadoria R$],0) + isnull(t0.[IMPOSTOS-NI],0) + isnull(t0.[DA Frete],0) + isnull(t0.[DA Seguro],0) + isnull(t0.[DA Outros],0) - isnull(t0.[IRF-TOTAL],0) ) as 'Faturamento Bruto',
				
			( isnull(t0.[Total Mercadoria R$],0) - isnull(t0.[IMPOSTOS-I],0) - isnull(t0.[IRF-TOTAL],0) ) as 'Faturamento Líquido',						
			...
			t0.[DA Base IRPJ],
			t0.[DA Base II],
			t0.[DA Base OUTROS IMP],		

			isnull(t0.[IRF-ISSF],0) as 'IRF-ISSF',
			isnull(t0.[IRF-IRRF],0) as 'IRF-IRRF',
			isnull(t0.[IRF-PIS],0) as 'IRF-PIS',
			isnull(t0.[IRF-COFINS],0) as 'IRF-COFINS',
			isnull(t0.[IRF-CSLL],0) as 'IRF-CSLL',			
			isnull(t0.[IRF-INSS],0) as 'IRF-INSS',
			isnull(t0.[IRF-10833],0) as 'IRF-10833',
			isnull(t0.[IRF-OUTROS],0) as 'IRF-OUTROS',
			isnull(t0.[IRF-TOTAL],0) as 'IRF-TOTAL',
			isnull(t0.[Base IRF-ISSF],0) as 'Base IRF-ISSF',
			isnull(t0.[Base IRF-IRRF],0) as 'Base IRF-IRRF',
			isnull(t0.[Base IRF-PIS],0) as 'Base IRF-PIS',
			isnull(t0.[Base IRF-COFINS],0) as 'Base IRF-COFINS',
			isnull(t0.[Base IRF-CSLL],0) as 'Base IRF-CSLL',			
			isnull(t0.[Base IRF-INSS],0) as 'Base IRF-INSS',
			isnull(t0.[Base IRF-10833],0) as 'Base IRF-10833',		
			isnull(t0.[Base IRF-OUTROS],0) as 'Base IRF-OUTROS',

			isnull(t0.[IRF-ISSF-P],0) as 'IRF-ISSF-P',
			isnull(t0.[IRF-IRRF-P],0) as 'IRF-IRRF-P',
			isnull(t0.[IRF-PIS-P],0) as 'IRF-PIS-P',
			isnull(t0.[IRF-COFINS-P],0) as 'IRF-COFINS-P',
			isnull(t0.[IRF-CSLL-P],0) as 'IRF-CSLL-P',			
			isnull(t0.[IRF-INSS-P],0) as 'IRF-INSS-P',
			isnull(t0.[IRF-10833-P],0) as 'IRF-10833-P',
			isnull(t0.[IRF-OUTROS-P],0) as 'IRF-OUTROS-P',
			isnull(t0.[IRF-TOTAL-P],0) as 'IRF-TOTAL-P',
			isnull(t0.[Base IRF-ISSF-P],0) as 'Base IRF-ISSF-P',
			isnull(t0.[Base IRF-IRRF-P],0) as 'Base IRF-IRRF-P',
			isnull(t0.[Base IRF-PIS-P],0) as 'Base IRF-PIS-P',
			isnull(t0.[Base IRF-COFINS-P],0) as 'Base IRF-COFINS-P',
			isnull(t0.[Base IRF-CSLL-P],0) as 'Base IRF-CSLL-P',			
			isnull(t0.[Base IRF-INSS-P],0) as 'Base IRF-INSS-P',
			isnull(t0.[Base IRF-10833-P],0) as 'Base IRF-10833-P',		
			isnull(t0.[Base IRF-OUTROS-P],0) as 'Base IRF-OUTROS-P',
