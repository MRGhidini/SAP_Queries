-- CRIA_FVT_RECDEV_5 -------------------------------------------------------------------------------------------------------------------------------
--
-- @CAN = '+CAN' TRAZ AS DEVOLUÇÕES CANCELADAS E SOMA ÀS DEVOLUÇÕES. NÃO MOSTRA AS LINHAS DAS RECDEVCAN (Tipo DOC), MAS TRAZ AS [Tipo DOC] = RECCAN 
-- @CAN ='-CAN' TRAZ A DEVULUÇÃO ORIGINAL, O CANCELAMENTO DELA, MAS NÃO TRAZ OS CANCELAMENTOS DAS NFS (Tipo DOC] = 'RECCAN'
--
--	Substituições				: [REC para [xxx									= Tabelas internas
--								: RECCAN para xxxCAN								= Referências ao documento Cancelamento
--								: RECDEV para xxxDEV								= Referências ao documento Devolução
--								: [ORPD para [xxxx									= Tabelas B1 docto marketing - cabec
--								: [RPD para [xxx									= Tabelas B1 docto marketing - linhas
--								: t2.[BaseType] = '21' para t2.[BaseType] = 'xx'	= Alterar para o Documento xx
--
---------------------------------------------------------------------------------------------------------------------------------------------------
if OBJECT_ID ('FVT_RECDEV_5') is not null
	drop function FVT_RECDEV_5
go
create function FVT_RECDEV_5
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime,
...
(
select	
		case when t0.CANCELED = 'C'	then 'RECDEVCAN'
			 when t0.SeqCode = 1 then 'RECCAN'	else 'RECDEV' end as 'Tipo DOC',
		...
		(-1)*((Select SUM(isnull(t10.LineTotal,0)) From [RPD3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [RPD13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		(case when t1.TaxOnly = 'Y' then 0.00 else (-1)*(t1.LineTotal)*((t0.DocTotal) / (t0.DocTotal + t0.DiscSum)) end ) as 'Total Mercadoria R$',
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RPD2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RPD13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
inner join [RPD12] as t2 on t2.DocEntry = t0.DocEntry
...
from [RPD1] as t1
inner join [RECDEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [RPD4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
from [RECDEV] as t2 		
inner join [RPD1] as t0 on t0.DocEntry = t2.DocEntry and t0.LineNum = t2.[Linha Doc]  
inner join [RPD5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [RECDEV_IRF_SUM] as t0 
group by t0.DocEntry2, t0.[Linha Doc2]
),

[RECDEV_FINAL] as 
(
select	(case when t0.[Tipo DOC] = 'RECDEVCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*
from [RECDEV] as t0 
left outer join [RECDEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [RECDEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [RECDEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
)
...
from [RECDEV_FINAL] as t0 
left outer join [RECDEV_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '21' and t2.[Tipo DOC]='RECDEVCAN' and @CAN = '+CAN')	
where	( t0.[Tipo DOC] = 'RECDEV' and @CAN = '+CAN' )
		or @CAN = '-CAN'   
