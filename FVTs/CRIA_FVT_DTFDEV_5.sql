-- CRIA_FVT_DTFDEV_5 ------------------------------------------------------------------------------------------------------------------------------
--
-- @CAN = '+CAN' TRAZ AS DEVOLUÇÕES CANCELADAS E SOMA ÀS DEVOLUÇÕES. NÃO MOSTRA AS LINHAS DAS DTFDEVCAN (Tipo DOC), MAS TRAZ AS [Tipo DOC] = DTFCAN 
-- @CAN ='-CAN' TRAZ A DEVULUÇÃO ORIGINAL, O CANCELAMENTO DELA, MAS NÃO TRAZ OS CANCELAMENTOS DAS NFS (Tipo DOC] = 'DTFCAN'
--
--	Substituições				: [DTF para [xxx									= Tabelas internas
--								: DTFCAN para xxxCAN								= Referências ao documento Cancelamento
--								: DTFDEV para xxxDEV								= Referências ao documento Devolução
--								: [ORPC para [xxxx									= Tabelas B1 docto marketing - cabec
--								: [RPC para [xxx									= Tabelas B1 docto marketing - linhas
--								: t2.[BaseType] = '14' para t2.[BaseType] = 'xx'	= Alterar para o Documento xx
--
--------------------------------------------------------------------------------------------------------------------------------------------------
if OBJECT_ID ('FVT_DTFDEV_5') is not null
	drop function FVT_DTFDEV_5
go
create function FVT_DTFDEV_5
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime,
...
(
select	
		case when t0.CANCELED = 'C'	then 'DTFDEVCAN'
			 when t0.SeqCode = 1 then 'DTFCAN'	else 'DTFDEV' end as 'Tipo DOC',
		...
		(-1)*((Select SUM(isnull(t10.LineTotal,0)) From [RPC3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [RPC13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RPC2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		(-1)*isnull((select SUM(isnull(t10.LineTotal,0)) from [RPC13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
inner join [RPC12] as t2 on t2.DocEntry = t0.DocEntry
...
left outer join [RPC1] as t10 on t10.DocEntry = t1.BaseEntry and t10.LineNum = t1.BaseLine and t1.BaseType = 19   /* liga ao doc original do cancelamento da devolução) */
where	(t0.DocDate between @DataIni and @DataFim) 
		and ( t0.BPLId is null or t0.BPLId between @BPLIdIni and @BPLIdFim)
		and ( (isnull(t1.BaseType,0) = 204 or isnull(t10.BaseType,0) = 204) ) 
),
...
from [RPC1] as t1
inner join [DTFDEV] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [RPC4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
from [DTFDEV] as t2 		
inner join [RPC1] as t0 on t0.DocEntry = t2.DocEntry and t0.LineNum = t2.[Linha Doc]  
inner join [RPC5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [ORPC] as t0 
inner join [RPC9] as t1 on t0.DocEntry = t1.DocEntry
Group by t0.DocEntry
),

[DTFDEV_FINAL] as 
(
select	(case when t0.[Tipo DOC] = 'DTFDEVCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*
from [DTFDEV] as t0 
left outer join [DTFDEV_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [DTFDEV_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]	
left outer join [DTFDEV_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [DTFDEV_ADTO] as t4 on t4.Docentry = t0.DocEntry
)
...
from [DTFDEV_FINAL] as t0 
left outer join [DTFDEV_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '19' and t2.[Tipo DOC]='DTFDEVCAN' and @CAN = '+CAN')	
where	( t0.[Tipo DOC] = 'DTFDEV' and @CAN = '+CAN' )
		or @CAN = '-CAN'   
