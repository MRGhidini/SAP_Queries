...
--
--	Substituições(CTRL H)		: _NFS para _xxx			= Nomes internos 
--								: [NFS para [xxx			= Tabelas internas
--								: 'NFS' para 'xxx'		= Referência ao docto NFS
--								: NFSCAN para xxxCAN	= Referências ao documento Cancelamento
--								: NFSDEV para xxxDEV	= Referências ao documento Devolução
--								: [OINV para [xxxx		= Tabelas B1 docto marketing - cabec
--								: [INV para [xxx			= Tabelas B1 docto marketing - linhas
--
--	Substituições Manuais		: t1.[BaseType] = (adequar para cada FVT)
--
-------------------------------------------------------------------------------------------------------

if OBJECT_ID ('FVT_NFS_5') is not null
	drop function FVT_NFS_5
go
create function FVT_NFS_5 
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime, 
...
(
select	
		case when t0.CANCELED <> 'C' then 'NFS' else 'NFSCAN' end as 'Tipo DOC',
		...
		(Select SUM(isnull(t10.LineTotal,0)) From [INV3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [INV13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		isnull((select SUM(isnull(t10.LineTotal,0)) from [INV2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
...
inner join [INV12] as t2 on t2.DocEntry = t0.DocEntry
...
from [INV1] as t1
inner join [NFS] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [INV4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		'Código da Despesa' as 'Código da Despesa' from [INV1] as t0 
inner join [INV5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [NFS_IRF_SUM] as t0 group by t0.DocEntry2, t0.[Linha Doc2]
),
...
from [OINV] as t0 
inner join [INV9] as t1 on t0.DocEntry = t1.DocEntry
Group by t0.DocEntry
),

[NFS_FINAL] as 
(
select (case when t0.[Tipo DOC] = 'NFSCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*  
from [NFS] as t0 
left outer join [NFS_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [NFS_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]
left outer join [NFS_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [NFS_ADTO] as t4 on t4.Docentry = t0.DocEntry
)
...
from [NFS_FINAL] as t0 
left outer join [FVT_NFSDEV_5] (@BPLIdIni, @BPLIdFim, @DataIni, @DataFim,'-CAN') as t1 
		on (t0.DocEntry = t1.DocEntryBase and t0.[Linha Doc] = t1.BaseLine and t1.[BaseType] = '13' and t1.[Tipo DOC]='NFSCAN' and @CAN = '+CAN')		
left outer join [NFS_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '13' and t2.[Tipo DOC]='NFSCAN' and @CAN = '+CAN')		
where  (t0.[Tipo DOC] ='NFS' and @CAN = '+CAN') or @CAN = '-CAN' 
)
