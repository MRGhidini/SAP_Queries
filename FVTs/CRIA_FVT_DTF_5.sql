...
--
--	Substituições(CTRL H)		: _DTF para _xxx			= Nomes internos 
--								: [DTF para [xxx			= Tabelas internas
--								: 'DTF' para 'xxx'		= Referência ao docto NFS
--								: DTFCAN para xxxCAN	= Referências ao documento Cancelamento
--								: DTFDEV para xxxDEV	= Referências ao documento Devolução
--								: [ODPO para [xxxx		= Tabelas B1 docto marketing - cabec
--								: [DPO para [xxx			= Tabelas B1 docto marketing - linhas
--
--	Substituições Manuais		: t1.[BaseType] = (adequar para cada FVT)
--
-------------------------------------------------------------------------------------------------------

if OBJECT_ID ('FVT_DTF_5') is not null
	drop function FVT_DTF_5
go
create function FVT_DTF_5 
	(	@BPLIdIni as int, 
		@BPLIdFim as int, 
		@DataIni as datetime, 
		@DataFim as datetime, 
...
(
select	
		case when t0.CANCELED <> 'C' then 'DTF' else 'DTFCAN' end as 'Tipo DOC',
		...
		(Select SUM(isnull(t10.LineTotal,0)) From [DPO3] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
																Where t10.DocEntry = t0.DocEntry and isnull(t11.ExpnsType,0) = 1) - 
		isnull((select SUM(isnull(t10.LineTotal,0)) from [DPO13] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode
		...
		isnull((select SUM(isnull(t10.LineTotal,0)) from [DPO2] as t10 inner join [OEXD] as t11 on t10.ExpnsCode = t11.ExpnsCode 
		...
		from [ODPO] as t0    
inner join [DPO1] as t1 on t1.DocEntry = t0.DocEntry
inner join [DPO12] as t2 on t2.DocEntry = t0.DocEntry
...
from [DPO1] as t1
inner join [DTF] as t2 on t2.DocEntry = t1.DocEntry and t2.[Linha Doc] = t1.LineNum	
inner join [DPO4] as t4 on t4.DocEntry = t1.DocEntry and t4.LineNum = t1.LineNum
...
select	t0.DocEntry as 'DocEntry2',
		...
		'Código da Despesa' as 'Código da Despesa' from [DPO1] as t0 
inner join [DPO5] as t1 on t1.AbsEntry = t0.DocEntry and t1.Doc1LineNo = t0.LineNum
--where t4.ExpnsCode = -1  /* exclui impostos despesas adicionais */
...
from [DTF_IRF_SUM] as t0 group by t0.DocEntry2, t0.[Linha Doc2]
),
...
from [ODPO] as t0 
inner join [DPO9] as t1 on t0.DocEntry = t1.DocEntry
Group by t0.DocEntry
),

[DTF_FINAL] as 
(
select (case when t0.[Tipo DOC] = 'DTFCAN' then -1 else 1 end) as 'S',
		t0.*,
		...
		t2.*  
from [DTF] as t0 
left outer join [DTF_TAX_SUM_2] as t1 on t0.DocEntry = t1.DocEntry1 and t0.[Linha Doc] = t1.[Linha Doc1]
left outer join [DTF_IRF_SUM_1] as t2 on t0.DocEntry = t2.DocEntry2 and t0.[Linha Doc] = t2.[Linha Doc2]
left outer join [DTF_DA_NDISTR] as t3 on t0.DocEntry = t3.DocEntry3 and t0.[Linha Doc] = t3.[Linha Doc3]
left outer join [DTF_ADTO] as t4 on t4.Docentry = t0.DocEntry
)
...
from [DTF_FINAL] as t0 
left outer join [FVT_DTFDEV_5] (@BPLIdIni, @BPLIdFim, @DataIni, @DataFim,'-CAN') as t1 
		on (t0.DocEntry = t1.DocEntryBase and t0.[Linha Doc] = t1.BaseLine and t1.[BaseType] = '18' and t1.[Tipo DOC]='DTFCAN' and @CAN = '+CAN')		
left outer join [DTF_FINAL] as t2 /* Cancelamento Novo */
		on (t0.DocEntry = t2.DocEntryBase and t0.[Linha Doc] = t2.BaseLine and t2.[BaseType] = '18' and t2.[Tipo DOC]='DTFCAN' and @CAN = '+CAN')		
where  (t0.[Tipo DOC] ='DTF' and @CAN = '+CAN') or @CAN = '-CAN' 
)
